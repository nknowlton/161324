...
function finalStateCard(linkage) {
  ...
  card.append(heading, dendrogram)
  return card
}
...
export function hierarchicalWidget() {
  ...
  const update = () => {
    ...
    if (!snapshot.lastMerge) {
      note.textContent = "Each observation starts as its own cluster. Move the slider to watch the agglomerative merges."
      return
    }

    note.textContent =
      `Latest merge: ${snapshot.lastMerge.leftSize} + ${snapshot.lastMerge.rightSize} points at distance ` +
      `${snapshot.lastMerge.height.toFixed(2)}. ${snapshot.clusters.length} clusters remain.`
  }
