...
monster_pam_lookup <- tibble(
  pam_cluster = unname(monster_pam$clustering),
  kmeans_cluster = unname(monster_kmeans$cluster)
) |>
...
monster_partition_compare <- monster_compare_points |>
  mutate(
    kmeans_cluster = unname(monster_kmeans$cluster),
    pam_cluster = unname(monster_pam$clustering)
) |>
  left_join(monster_pam_lookup, by = "pam_cluster") |>
  mutate(
    agreement = if_else(kmeans_cluster.x == kmeans_cluster.y, "Same cluster after relabelling", "Different cluster"),
    agreement = factor(agreement, levels = c("Same cluster after relabelling", "Different cluster"))
  ) |>
  rename(
    kmeans_cluster = kmeans_cluster.x,
    pam_relabelled_cluster = kmeans_cluster.y
  )
...
- After fitting, we can compare those cluster labels to the known `type` variable to see what structure the clustering recovered.
