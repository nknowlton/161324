...
DBSCAN) addresses this by considering a whole range of density
thresholds and constructing a hierarchy of density-based clusters. The
final clusters are then extracted from those parts of the hierarchy that
are most stable across density levels. Conceptually this makes HDBSCAN
closer to hierarchical clustering than to the fixed-threshold DBSCAN
algorithm.

When the data are high-dimensional, it is often useful to first produce
a low-dimensional display. UMAP (Uniform Manifold Approximation and
Projection) is not itself a clustering method. Rather, it constructs a
low-dimensional map that attempts to preserve local neighbourhood
relationships from the original feature space. In the notation of the
lecture, if $y_i$ are low-dimensional coordinates and $w_{ij}$ are
weights describing neighbourhood strength in the original data, then the
low-dimensional affinities may be written as
$$v_{ij} = \left(1 + a \| y_i - y_j \|^{2b} \right)^{-1},$$ and the
coordinates are chosen to minimise a cross-entropy objective of the form
$$\mathcal{L} =
\sum_{i \neq j}
\left[
...
\right].$$
The resulting `UMAP 1` and `UMAP 2` axes are therefore display
coordinates rather than original variables. This is useful for plotting
clusters, but the clustering itself should still be fit in the original
